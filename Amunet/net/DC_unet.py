...
class DC_unet(nn.Module):
    def __init__(self, n_classes, in_channels, is_deconv, is_batchnorm):
        ...
        self.down1 = unetDown1(self.in_channels, filters[0], self.is_batchnorm)

        self.down2 = unetDown(filters[0], filters[1], self.is_batchnorm)
        self.down3 = unetDown1(filters[1], filters[2], self.is_batchnorm)
        ...
        self.final = nn.Sequential(nn.Conv2d(filters[0], self.n_classes,kernel_size=(3, 1), stride=(2, 1), padding=(1, 0)))
    ...
    def forward(self, inputs):
        ...
        x = F.interpolate(up1, size=[402, 301], mode='bilinear', align_corners=False)
        out = self.final(x)
...
if __name__ == '__main__':
    ...
    # FCNVMB
    x = torch.zeros((10, 29, 400, 300))
    model = DC_unet(n_classes=1, in_channels=29, is_deconv=True, is_batchnorm=True)
    out = model(x)
    print("out: ", out.size())
