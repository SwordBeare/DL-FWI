...
class SeismicRecordDownSampling(nn.Module):
    ...
    def __init__(self, shot_num):
        ...
        self.pre_dim_reducer1 = ConvBlock(shot_num, 32, kernel_size=(3, 1), stride=(2, 1), padding=(1, 0))
        self.pre_dim_reducer2 = ConvBlock(32, 32, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))

    def forward(self, dimred0):
        dimred0 = F.interpolate(dimred0, size=(402,301), mode='bilinear', align_corners=False)
...
class unetUp(nn.Module):
    ...
    def forward(self, inputs1, inputs2):
        ...
        inputs1 = F.interpolate(inputs1, size=[inputs2.shape[2],inputs2.shape[3]], mode='bilinear', align_corners=False)


        return self.conv(torch.cat([inputs1, inputs2], 1))


class bianxin_conv_CBAM(nn.Module):
    def __init__(self, n_classes, in_channels, is_deconv, is_batchnorm):
        ...
        :param is_batchnorm:    Whether to use BN
        '''
        super(bianxin_conv_CBAM, self).__init__()
        ...
        self.conv_0 = SeismicRecordDownSampling(29)

        self.conv_1 = unetConv11(filters[0], filters[0], self.is_batchnorm)

        self.conv_2 = unetConv2(filters[0], filters[1], self.is_batchnorm)

        self.conv_3 = unetConv11(filters[1], filters[2], self.is_batchnorm)

        self.conv_4 = unetConv2(filters[2], filters[3], self.is_batchnorm)
    ...
    def forward(self, inputs):
        ...
        :return:
        '''
        data = self.conv_0(inputs)

        data1_1 = self.conv_1(data)
        data1 = self.down(data1_1)

        data2_1 = self.conv_2(data1)
        data2 = self.down(data2_1)

        data3_1 = self.conv_3(data2)
        data3 = self.down(data3_1)

        data4_1 = self.conv_4(data3)
        data4 = self.down(data4_1)

        center = self.center(data4)

        center = self.cbam(center)

        up4 = self.up4(center, data4_1)
        up3 = self.up3(up4, data3_1)
        up2 = self.up2(up3, data2_1)
        up1 = self.up1(up2, data1_1)
...
if __name__ == '__main__':
    # FCNVMB
    x = torch.zeros((10, 29, 400, 301))
    model = bianxin_conv_CBAM(n_classes=1, in_channels=29, is_deconv=True, is_batchnorm=True)
    out = model(x)
    print("out: ", out.size())
